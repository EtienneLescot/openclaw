    ...
    api.registerHttpRoute({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const summary = summarizeSupervisorConversation(body.history);
          ...
          const largeLlmOutput =
            !isNoActionDecision && result.text && result.text.trim().length > 0
              ? result.text
              : "[NO_ACTION or empty]";

          api.logger.info(
            formatSupervisorConversationLog({
              roomName: body.roomName,
              channel,
              userTranscript: summary.userTranscript,
              smallLlmOutput: summary.smallLlmOutput,
              largeLlmOutput,
              structured: Boolean(result.decision),
...
type SupervisorConversationSummary = {
  userTranscript: string;
  smallLlmOutput: string;
};
...
type SupervisorConversationLog = {
  roomName: string;
  channel: string;
  userTranscript: string;
  smallLlmOutput: string;
  largeLlmOutput: string;
  structured: boolean;
  action?: "NO_ACTION" | "TRIGGER";
  reason?: string;
};

function summarizeSupervisorConversation(input: string): SupervisorConversationSummary {
  const history = extractConversationHistoryForLogs(input).trim();
  const turns = parseConversationTurns(history);

  const latestUser = findLatestRole(turns, ["user", "human"]);
  const latestNonUser = [...turns].reverse().find((turn) => {
    const role = normalizeRole(turn.role);
    return role !== "user" && role !== "human";
  });

  return {
    userTranscript: latestUser?.text ?? truncateForLog(history, 800),
    smallLlmOutput: latestNonUser?.text ?? "[none]",
  };
}

function parseConversationTurns(input: string): ParsedTurn[] {
  const fromJson = parseConversationTurnsFromJson(input);
  if (fromJson.length > 0) return fromJson;
  return parseConversationTurnsFromLines(input);
}

function parseConversationTurnsFromJson(input: string): ParsedTurn[] {
  ...
  for (const message of rawMessages) {
    ...
    const role = readString(message, ["role", "speaker", "author"]) ?? "unknown";
    const text = readMessageText(message);
    if (!text) continue;
    turns.push({ role, text: truncateForLog(text, 800) });
  }

  return turns;
}

function parseConversationTurnsFromLines(input: string): ParsedTurn[] {
  const lines = input
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean);
  ...
  const turns: ParsedTurn[] = [];
  const rolePrefixes: Array<{ role: string; regex: RegExp }> = [
    { role: "user", regex: /^(user|human|transcript)\s*:\s*(.+)$/i },
    {
      role: "assistant",
      regex: /^(assistant|agent|small\s*llm|fast\s*llm|voiceagent|voice\s*agent)\s*:\s*(.+)$/i,
    },
  ];

  let activeTurn: ParsedTurn | null = null;
  for (const line of lines) {
    let matchedPrefix = false;
    for (const prefix of rolePrefixes) {
      const match = line.match(prefix.regex);
      if (!match) continue;
      matchedPrefix = true;
      const text = truncateForLog(match[2] ?? "", 800);
      if (!text) break;
      activeTurn = { role: prefix.role, text };
      turns.push(activeTurn);
      break;
    }

    if (!matchedPrefix && activeTurn) {
      activeTurn.text = truncateForLog(`${activeTurn.text}\n${line}`, 800);
    }
  }
...
function readString(value: object, keys: string[]): string | null {
...
function readMessageText(message: object): string {
  const record = message as Record<string, unknown>;
  const directText = readString(record, ["text", "message", "content"]);
  ...
  const textParts = content
    .map((part) => {
      ...
      return readString(part, ["text", "content", "value"]) ?? "";
    })
    .filter(Boolean);
...
function findLatestRole(turns: ParsedTurn[], roles: string[]): ParsedTurn | undefined {
  const normalizedRoles = new Set(roles.map(normalizeRole));
  return [...turns].reverse().find((turn) => normalizedRoles.has(normalizeRole(turn.role)));
}
...
function formatSupervisorConversationLog(log: SupervisorConversationLog): string {
  const statusLabel = log.structured ? "yes" : "no";
  const statusColor = log.structured ? ansi.success : ansi.warn;
  const action = log.structured ? (log.action ?? "n/a") : "n/a";
  const reason =
    log.structured && typeof log.reason === "string" && log.reason.trim().length > 0
      ? truncateForLog(log.reason, 240)
      : "n/a";

  return [
    `${ansi.heading("[stimm-voice] Supervisor conversation")} ${ansi.dim(`room=${log.roomName} channel=${log.channel}`)}`,
    `${ansi.user("USER")}: ${log.userTranscript}`,
    `${ansi.small("SMALL_LLM")}: ${log.smallLlmOutput}`,
    `${ansi.large("LARGE_LLM")}: ${log.largeLlmOutput}`,
    `${ansi.muted("structured_json")}: ${statusColor(statusLabel)}`,
    `${ansi.muted("action")}: ${ansi.info(action)}`,
    `${ansi.muted("reason")}: ${reason}`,
  ].join("\n");
}
...
function ansiWrap(code: string, value: string): string {
  if (!supportsAnsi) return value;
  return `\u001b[${code}m${value}\u001b[0m`;
}

const ansi = {
  heading: (value: string) => ansiWrap("1;36", value),
  user: (value: string) => ansiWrap("1;32", value),
  small: (value: string) => ansiWrap("1;34", value),
  large: (value: string) => ansiWrap("1;35", value),
  success: (value: string) => ansiWrap("32", value),
  warn: (value: string) => ansiWrap("33", value),
  info: (value: string) => ansiWrap("36", value),
  muted: (value: string) => ansiWrap("2", value),
  dim: (value: string) => ansiWrap("2", value),
};
